...
def upgrade() -> None:
    ...
    op.execute(
        "ALTER TABLE post ADD COLUMN content_new JSONB"
    )
    op.execute(
        "UPDATE post SET content_new = content_old::jsonb"
    )
...
def downgrade() -> None:
    ...
    op.execute(
        "ALTER TABLE post ADD COLUMN content_new TEXT"
    )
    op.execute(
        "UPDATE post SET content_new = content_old::text"
    )
    op.drop_column('post', 'content_old')
    op.alter_column('post', 'content_new', new_column_name='content')
